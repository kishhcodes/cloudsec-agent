...
class AuditReport:
    ...
    def __init__(self, cloud_provider: str, project_id: str, output_dir: str = "reports"):
        ...
        # Enhanced features
        self.include_charts = True  # Enable charts by default
        self.include_compliance = []  # List of compliance frameworks to include
    ...
    def _build_summary(self, styles) -> List:
        ...
        # Calculate security score if not done yet
        ...
        <br/>
        <b>Security Score: {self.security_score:.1f}/100</b><br/>
        ...
        if self.include_charts:
            ...
            try:
                score_chart = self.chart_generator.security_score_gauge(self.security_score)
                story.append(Image(io.BytesIO(score_chart), width=5*inch, height=3.75*inch))
                story.append(Spacer(1, 0.2*inch))
            except Exception as e:
                console.print(f"[yellow]Warning: Could not generate security score chart: {e}[/yellow]")
            
            # Severity distribution pie chart
            try:
                if findings:
                    pie_chart = self.chart_generator.severity_distribution_pie(findings)
                    story.append(Image(io.BytesIO(pie_chart), width=5*inch, height=3.75*inch))
                    story.append(Spacer(1, 0.2*inch))
            except Exception as e:
                console.print(f"[yellow]Warning: Could not generate pie chart: {e}[/yellow]")
            
            # Category bar chart
            try:
                if self.sections:
                    bar_chart = self.chart_generator.findings_by_category_bar(self.sections)
                    story.append(Image(io.BytesIO(bar_chart), width=5*inch, height=3.75*inch))
                    story.append(Spacer(1, 0.2*inch))
            except Exception as e:
                console.print(f"[yellow]Warning: Could not generate bar chart: {e}[/yellow]")
        
        # Add compliance section if enabled
        if self.include_compliance:
            findings = self.get_all_findings()
            story.append(PageBreak())
            story.append(Paragraph("Compliance Framework Assessment", styles['Heading_Custom']))
            story.append(Spacer(1, 0.2*inch))
            ...
            for framework_name in self.include_compliance:
                coverage = self.compliance_mapper.calculate_framework_coverage(
                    findings,
                    framework_name
                )
            ...
            if len(compliance_table_data) > 1:
                compliance_table = PDFTable(compliance_table_data, colWidths=[3*inch, 1.5*inch, 1*inch])
    ...
    def add_database_analysis(self, analysis: Dict[str, Any]) -> None:
        """Add database security analysis section"""
        self.add_section("Database Security Analysis", analysis)
        self._update_summary(analysis)
    ...
    def calculate_security_score(self) -> float:
        ...
        """
        if self.summary_data['total_findings'] == 0:
        ...
        score -= self.summary_data['critical_issues'] * 10     # -10 per critical
        score -= self.summary_data['high_risk'] * 5            # -5 per high
        score -= self.summary_data['medium_risk'] * 2          # -2 per medium
        score -= self.summary_data['low_risk'] * 0.5           # -0.5 per low
        
        # Add points for compliant items (cap at 100)
        compliant_bonus = (self.summary_data['compliant_items'] / 
                          max(1, self.summary_data['total_findings'])) * 5
    ...
    def get_all_findings(self) -> List[Dict[str, Any]]:
        ...
        findings = []
        for section in self.sections:
            section_findings = section.get('content', {}).get('findings', [])
            if isinstance(section_findings, list):
                findings.extend([f for f in section_findings if isinstance(f, dict)])
        return findings
    
    def enable_compliance_mapping(self, frameworks: Optional[List[str]] = None) -> None:
        """
        Enable compliance framework mapping.
        
        Args:
            frameworks: List of framework names to include (None = all)
        """
        if frameworks:
            self.include_compliance = frameworks
        else:
...
class AzureAuditReport(AuditReport):
    """Azure-specific audit report"""
    
    def __init__(self, subscription_id: str, output_dir: str = "reports"):
        super().__init__("azure", subscription_id, output_dir)
